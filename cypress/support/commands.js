...
Cypress.Commands.add("CompleteQuestionnaire", () => {
    ...
    // State
    cy.get(':nth-child(6) > [data-v-32b51562=""] > .cdp-field > .cdp-input').should('not.be.disabled').type('MA')
    // Height Feet
    cy.get(':nth-child(7) > [data-v-32b51562=""] > .cdp-field > .cdp-input').should('not.be.disabled').select('5')
    // Height Inches
    cy.get(':nth-child(8) > [data-v-32b51562=""] > .cdp-field > .cdp-input').should('not.be.disabled').select('8')
    // Weight Unit Lbs
    ...
    cy.get('body').then(($body) => {
        if ($body.find(':nth-child(4) > [data-v-32b51562=""] > .cdp-field > .cdp-input').length > 0) {
            // DOB
            cy.get(':nth-child(4) > [data-v-32b51562=""] > .cdp-field > .cdp-input').type('1998-09-09')
        }
        ...
        if ($body.find(':nth-child(15) > [data-v-32b51562=""] > .cdp-field').length > 0) {
            // Other Preferences
            cy.get(':nth-child(15) > [data-v-32b51562=""] > .cdp-field').type('N/A')
        }
        if ($body.find(':nth-child(16) > [data-v-32b51562=""] > :nth-child(1) > :nth-child(5) > .radio-container > .checkmark').length > 0) {
            // Style of diet
            cy.get(':nth-child(16) > [data-v-32b51562=""] > :nth-child(1) > :nth-child(5) > .radio-container > .checkmark').click()
        }
...
Cypress.Commands.add("VerifyOrders", (_file) => {
    ...
    // Navigate to Admin Tool logiin page
                ...
                for (let i = 1; i < n; i++) {
                    cy.get('.th-name').eq(i).invoke('text').then((name) => {
                        for (let x = data.length - 1; x > 0; x--) {
                            ...
                            if (name.includes('(sku_move_int)')) {
                                itemName = name.trim().replace(/[\n(sku_move_int)]/g, '')
                            } else if (name.includes('(sku_ETz4R4WE6B0Fuf)')) {
                                itemName = name.trim().replace(/[\n(sku_ETz4R4WE6B0Fuf)]/g, '')
                            } else {
                                itemName = name.trim().replace(/[\n]/g, '')
                            }
